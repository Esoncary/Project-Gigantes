...
public class PlayerController : MonoBehaviour
{
    ...
    public float releaseBufferTimer;//释放缓冲计时器
    ...
    private void Update()
    {
        ...
        if (ReleaseInputDown)
        {
            releaseBufferTimer = releaseBufferTime;
        }
    ...
    #region 释放输入缓冲
    /// <summary>
    /// 检查是否可以触发释放（考虑冷却和输入缓冲）
    /// </summary>
    public bool CanTriggerRelease()
    {
        if (releaseCoolingTimer > 0) return false;
        if (ReleaseInputDown || releaseBufferTimer > 0) return true;
        return false;
    }
    ...
    /// 消耗释放输入缓冲（进入释放状态时调用）
    /// </summary>
    public void ConsumeReleaseBuffer()
    {
        releaseBufferTimer = -1;
    }
    #endregion
}